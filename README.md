...
editorial are, and the LeetCode track opens leetcode.com. Anything else makes one of the
two tracks a list of links to the other site.

NeetCode serves most problems under its own renamed slugs — `two-sum` lives at
`/problems/two-integer-sum` — so `packages/catalog/data/neetcode-slugs.json` maps between
them, generated by `pnpm neetcode:map` from NeetCode's sitemap and its published rename
table.
...
Settings can pin both tracks to one site instead, in either direction.
